<form class="md:w-full space-y-5" wire:submit.prevent='editarVacante'>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 gap-y-4">
       ...
       <!-- Organización Existente -->
        @auth
            @if ($orgexistentes->isNotEmpty())
                <div>
                    <x-input-label for="organizacion" :value="__('Seleccionar Organización')" class="mb-2" />
                    <select id="organizacion" 
                        class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm transition-colors duration-200 hover:border-indigo-400" 
                        wire:model="selectedOrganizacion">
                        
                        <!-- Opción "Sin Organización" -->
                        <option value="" @if(is_null($selectedOrganizacion)) selected @endif>{{ __('Sin Organización') }}</option>
                        
                        <!-- Listado de organizaciones -->
                        @foreach ($orgexistentes as $organizacion)
                            <option value="{{ $organizacion->id }}" @if($selectedOrganizacion == $organizacion->id) selected @endif>
                                {{ $organizacion->nombre }} 
                            </option>
                        @endforeach
                    </select>
        ...
        <!-- Área de carga (se muestra solo cuando no hay imagen nueva) -->
        <div class="{{ $imagen_nueva && !$errors->has('imagen_nueva') ? 'hidden' : 'block' }}">
            <x-input-label for="imagen_nueva" :value="__('Imagen destacada')" />
        ...
        <!-- Vista previa de la imagen NUEVA si se sube -->
            ...
            <div class="relative mt-1 h-40 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                ...
                <div class="absolute bottom-0 left-0 right-0 bg-white/90 px-3 py-1.5 text-xs">
                    <div class="flex items-center justify-between">
                        <span class="truncate font-medium">{{ $imagen_nueva->getClientOriginalName() }}</span>
                        <span>{{ round($imagen_nueva->getSize() / 1024) }}KB</span>
                    </div>
                </div>
            </div>
        @endif

        <!-- Imagen actual si no se ha subido una nueva -->
        ...
        @enderror
            ...
            <!-- Indicador de carga -->
            <div wire:loading wire:target="imagen_nueva" 
